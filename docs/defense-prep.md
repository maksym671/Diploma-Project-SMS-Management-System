...
Раньше и `healthCheckPath` в `render.yaml`, и GitHub Action дёргали `/login/` — страницу, которая рендерится без единого обращения к базе. Теперь оба смотрят на `/healthz/`, который выполняет `SELECT 1`, так что проба будит и веб-сервис, и Postgres.

Что это дало, измерено: после **девяти минут** полного простоя первый запрос к `/healthz/` показал `db=92 мс` — ровно столько же, сколько на прогретом сайте, то есть база не спала. Зато время самого приложения подскочило с обычных 110 мс до 758 мс, и на дашборде первый ответ занял 1.78 с вместо 0.92 с. Второй запрос сразу вернулся к норме.

Вывод для защиты: остаточная задержка после простоя — это троттлинг CPU на бесплатном тарифе Render, а не пробуждение базы, и лечится она только трафиком. **Открой сайт за одну-две минуты до выхода к комиссии и кликни пару страниц** — дальше он держит 0.5–0.9 с на страницу.
...
111. Модели, логин, изоляция преподавателей, ёмкость курса, веса оценок, Mark
Class, authorship, i18n, `seed_demo`, дым каждой страницы для обеих ролей,
бюджет запросов дашборда. Плюс группы, добавленные после ручного прогона
интерфейса и работы над скоростью: статус 422 на невалидную форму,
оформленные 404/500, каталог переводов без fuzzy-записей, политика соединений
с базой, health-проба, кеш сессий и заголовок Server-Timing. CI: тесты +
`makemigrations --check` + `check --deploy` + collectstatic.
