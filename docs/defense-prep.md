...
Засыпают двое: веб-сервис Render и вычислитель Neon. Пинговать `/login/` было мало — эта страница рендерится без единого запроса в базу, так что Postgres всё равно уходил в сон, и первый клик по данным упирался в его пробуждение. GitHub Action каждые 10 минут дёргает `/healthz/`, который выполняет `SELECT 1`; туда же смотрит `healthCheckPath` в `render.yaml`, так что и собственные проверки Render держат базу тёплой. Планировщик GitHub не гарантия — для защиты всё равно открой сайт за 1–2 минуты до выхода.
